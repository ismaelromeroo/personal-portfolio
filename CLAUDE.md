...
- **Media placeholders.** `Project.media` may point at `/placeholder.svg?...`. Components check for this placeholder path (see `ProjectCard`'s `hasMedia`) to decide whether to render an image or a text fallback. Real assets go in `/public` (photos under `/public/images`, logos under `/public/logos`, favicons under `/public/icons`) and are wired up by editing the paths in `portfolio-data.ts`.
